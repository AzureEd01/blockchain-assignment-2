...
def mine_block(record_str, difficulty, log):
    prefix = '0' * difficulty
    nonce = 0
    start_time = time.time()

    log(f"[MINING] Starting mining...")

    while True:
        text = record_str + str(nonce)
...
# ================= UI ===================
root = tk.Tk()
root.title("Inventory Proof of Work Demo")
...
tk.Label(root, text="Console Output:").grid(row=5, column=0, columnspan=2)
output_console = scrolledtext.ScrolledText(root, height=10, width=60, state='normal')
output_console.grid(row=6, column=0, columnspan=2, padx=5)

# Inventory Display
tk.Label(root, text="Node Inventories:").grid(row=7, column=0, columnspan=2)
inventory_display = scrolledtext.ScrolledText(root, height=12, width=60)
inventory_display.grid(row=8, column=0, columnspan=2, padx=5)
